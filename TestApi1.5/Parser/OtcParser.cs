...
namespace TestApi.Parser
{
    public class OtcParser : AbstractWebSiteParser
    {
        public override async Task<List<Supplier>> Parse(string okpd2)
        {
            ...
            using (var context = BrowsingContext.New(Configuration.Default.WithMetaRefresh().With(requester).WithDefaultLoader(new LoaderOptions() { IsResourceLoadingEnabled = true })))
            {
